# AGENTS.md — AgentForge / 智造工坊

> 本文件是 coding agent 在本仓库工作时的契约。Claude Code 通过 `CLAUDE.md` 加载，Codex/Kimi/Copilot 直接读本文件。

## 仓库目的

AgentForge 是「企业 AI Agent 定制实施服务」的顾问端工作台。读两份现金流报告（已嵌入 README）能理解为什么这个产品存在、要服务什么客户、要解决什么痛点。
...
- **数据库可见性**：所有数据放 SQLite 单文件 `data/agentforge.db`，schema 用 TypeScript 集中维护。任何字段变更要更新 `src/lib/schema.ts` 顶部的 CHANGELOG 注释。
